...
//   const Helper = require('../utils/helper');
  ...
  const save = async (payload) => {
    const {branchName, branchCode,//userId,
       branchDescription,status,createdBy
    } =
      payload;
    ...
    const isExist = await BranchesModel.findOne({ where: { branch_code: branchCode }, transaction });
  
      if (isExist) {
        await transaction.rollback();
  
        return { errors: [ { name: 'branchCode', message: 'duplicate entry.' } ] };
      }
  
      const isExistBranchName = await BranchesModel.findOne({ where: { branch_name: branchName }, transaction });
  
      if (isExistBranchName) {
        await transaction.rollback();
  
        return { errors: [ { name: 'branchName', message: 'duplicate entry.' } ] };
      }
  console.log("doc",doc)
    const Branchs = await BranchesModel.create({
  ...
  const update = async (payload) => {
    const {
      publicId,   branchName, branchCode,branchDescription, ...data
    } = payload;
  ...
  const getList = async (payload) => {
    const {
      role, pageSize, pageNumber, filters, sorting,
    } = payload;
  ...
  const getListStatus = async (payload) => {
    ...
    const attributes =  ['status', [sequelize.fn('COUNT',sequelize.col('status')), 'count']];
